...
1.在用React的时候用过最原始的CSS写法，这种写法需要让每个样式都具有唯一性，最终引入styles里面是不做任何修改的。

2.后面使用[CSS Modules](https://github.com/css-modules/css-modules)可以让组件className控制权转交给JS，我们不会去关心命名冲突的问题，同时可以灵活控制生成的命名，同时引入的成本接近于0，还可以用预处理器跟后处理器。

3.`styled-components`可以让CSS真正意义上的写到JS里面，
